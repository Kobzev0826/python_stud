...
result_parsing = pd.DataFrame()

#начинаем поиск по каждому каталогу 
import globus_items as gl

Data_Frame = pd.DataFrame()
for url in list_urls: 
    Data_Frame = gl.find_products(base_url + url)
    print(Data_Frame.head())
    result_parsing = result_parsing.append(Data_Frame)
    
result_parsing.to_excel('Parsing_result.xlsx')
